...
- A membership that no attribute holds is not reported as revoked when the connector can see that it exists: a membership that comes from the user's own `gidNumber` (the group is a primary group) and a membership inherited through a nested group each return an explanatory error, because a membership write to this group cannot remove either. The same check runs after a revoke has removed a direct membership, so a principal who was both a direct member and a primary-group or inherited member is reported with what remains instead of a success the next sync would contradict. Only when neither applies is the result a plain success (a direct membership was removed) or `GrantAlreadyRevoked` (there was none).

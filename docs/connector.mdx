...
- The same attribute cannot be given the same value in both directions; attribute names must not be empty; and `objectClass` and password attributes cannot be used as the marker.
- Each action returns `success`, `status` (`"enabled"` / `"disabled"`), `applied` (the number of attributes changed; `0` means the account was already in the requested state), `skipped`, and `updated_user` (a best-effort re-fetch of the user resource after the write). The connector re-reads the entry and verifies the configured attributes now hold their configured values — or are absent, for a configured clear — before reporting success.
...
- **Quote the values.** An unquoted `TRUE`/`FALSE` is read as a YAML boolean and written in lowercase, which LDAP's Boolean syntax (RFC 4517) rejects. `Y` and `N` are not YAML booleans and are safe unquoted, but quoting every value avoids the trap.

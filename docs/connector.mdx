...
- Each action returns `success`, `status` (`"enabled"` / `"disabled"`), `applied` (the number of attributes changed; `0` means the account was already in the requested state), and `updated_user` (a best-effort re-fetch of the user resource after the write). The connector re-reads the entry and verifies the configured attributes now hold their configured values — or are absent, for a configured clear — before reporting success. An attribute it cannot write (the entry's RDN attribute, for example) fails the action rather than being reported as skipped, on the first call and on every retry alike.

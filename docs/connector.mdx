...
| `update_user_attrs` | `resource_id` (string, required), `attrs` (string map, required), `attrs_update_mask` (string list, required) | Set or clear arbitrary LDAP attributes on an existing user. `resource_id` is the user's DN; `attrs` maps attribute name to value (an empty value clears the attribute); only names listed in `attrs_update_mask` are written. This action always targets users. It also backs C1's [push profile](/product/admin/account-provisioning) flow for LDAP users. |
...
| Action name | Additional fields | Description |
| ----------- | ----------------- | ----------- |
| `update_profile` | `user_id` (resource ID, required), `first_name` (string), `last_name` (string), `display_name` (string), `email` (string), `custom_attributes` (string map) | Set core profile fields (first name, last name, display name, email) and/or arbitrary custom LDAP attributes on an existing user. Unlike `update_user_attrs`, this action is scoped to the `user` resource type, which is what makes it discoverable and usable from C1's attribute-push-rule feature. |
...
<Note>
  `update_profile` applies the following rules:
  ...
  - **Custom attributes:** `custom_attributes` maps arbitrary raw LDAP attribute names to values; an empty value clears the attribute (unlike the named fields above, where empty just means "not supplied").
  - **Collisions:** a `custom_attributes` key that case-insensitively matches one of the four named fields is dropped (never merged with, or overwriting, the named field's slot) and reported once in `skipped`.
  - **Not modifiable:** the same restrictions as `update_user_attrs` apply -- password attributes (`userPassword`, or any name containing `password`), `objectClass`, and the user's RDN attribute cannot be modified through this action.
